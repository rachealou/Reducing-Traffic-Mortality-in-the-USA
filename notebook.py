...
# typically, we can read_csv and .head() the file
...
# features are the dependent variables
...
# the regression coefficients represent the value at which the 
...
# StandardScaler() standardizes a feature by subtracting the mean and 
# then scaling to unit variance
# features is defined as a unit in python
scaler = StandardScaler()
# fit centres the data (zero mean and unit standard error)
# transform applies transformations of standardization onto the set of data
# fit_transform() does both of fit and transform
# features are the dependent variable defined above
features_scaled = scaler.fit_transform(features)

# Import the PCA class function from sklearn
# PCA clusters are highly correlated with each other
# the axis are ranked in order of importance
# eg. distance between clusters horizontally (PC1) are more different
# than those that are vertical (PC2)
# 
...
# the elbow of the scree plot are the points before the line flattens
...
for k in ks:
    ...
    # KMeans refers to the number of clusters and are formed using # and random state
...
# inertias is a witih-cluster sum-of-squares variable
# inertias also signifies the sum of distances from your sample points to the cluster sample
...
# a violin plot is like a box plot with the density of each component visually illustrated through a 'violin' shape
